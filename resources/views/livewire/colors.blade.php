<main>
    <div class="p-4 bg-white block sm:flex items-center justify-between lg:mt-1.5 lg:pt-12">
        <div class="mb-1 w-full">
            <div class="mb-4">
                <h1 class="text-xl sm:text-2xl font-semibold text-gray-900">Colores</h1>
            </div>
            <div class="sm:flex">
                        ...
                        @foreach($colors as $color)
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="p-4 whitespace-nowrap text-base font-medium">
                                    <div class="w-full h-4" style="background-color: {{ $color->name }}"></div>
                                </td>
                                <td class="p-4 whitespace-nowrap space-x-2">
                                    <button wire:click="edit({{ $color->id }})" type="button" data-modal-toggle="user-modal" class="text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm inline-flex items-center px-3 py-2 text-center">
    ...
    <x-modal :open="$showModal" cleanAction="resetValues">
        <x-slot:title>
            {{ $isEdit ? 'Editar Color' :'Agregar Color'}}
        </x-slot:title>
